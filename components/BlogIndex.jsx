  ...
  return (
    <div className="md:gap-8 md:columns-3 md:space-y-10">
      {blog.map((blog) => (
        <div
          key={blog.id}
          className="card h-auto md:h-[450px] shadow-xl hover:-translate-y-3 transition delay-100 my-8 md:my-auto pb-2"
        >
          ...
          <ul>
            <li key={blog.id} className="card-body px-5 py-2">
              ...
              <div className="card-actions justify-end ">
                {blog.tags.map((tag) => (
                  <div className="badge badge-accent text-xs" key={tag}>
                    {tag}
                  </div>
                ))}
              </div>
